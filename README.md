...
Live `servers.json` includes these children after Railway vars were set 2026-09-03. A crashing stdio child still takes down the whole mcp-proxy. Metricool and n8n stay as their own HTTP URLs. `memory` is the official `@modelcontextprotocol/server-memory` graph; Hermes should keep it disabled (see Hermes `docs/mcp-aggregator/memory.md`).
